...
echo "Setting network speed to 75mbit for 30 seconds via tc"
sudo tc qdisc add dev lo root handle 1: htb default 10
sudo tc class add dev lo parent 1: classid 1:1 htb rate 75000kbit
sudo tc class add dev lo parent 1:1 classid 1:10 htb rate 75000kbit
...
echo "Setting network speed to 15mbit for 30 seconds via tc"
sudo tc qdisc del dev lo root
sudo tc qdisc add dev lo root handle 1: htb default 10
sudo tc class add dev lo parent 1: classid 1:1 htb rate 15000kbit
sudo tc class add dev lo parent 1:1 classid 1:10 htb rate 15000kbit
sudo tc qdisc add dev lo parent 1:10 handle 10: sfq perturb 10
sudo tc filter add dev lo parent 1:0 protocol ip prio 1 u32 \
match ip dport 80 0xffff flowid 1:10
sleep 30s
